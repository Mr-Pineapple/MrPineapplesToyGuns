...
public class EntityRegistry {
    ...
    public static final DeferredRegister<EntityType<?>> ENTITY_REGISTRY = DeferredRegister.create(ForgeRegistries.ENTITY_TYPES, ToyGuns.ID);

    //My Dart projectile being registered by the helper method below
    public static final RegistryObject<EntityType<DartEntity>> DART = registerProjectile("dart", DartEntity::new);
    ...
    private static <T extends ProjectileEntity> RegistryObject<EntityType<T>> registerProjectile(String id, BiFunction<EntityType<T>, Level, T> function) {
        return ENTITY_REGISTRY.register(id, () -> EntityType.Builder.of(function::apply, MobCategory.MISC)
                .sized(0.25F, 0.25F)
